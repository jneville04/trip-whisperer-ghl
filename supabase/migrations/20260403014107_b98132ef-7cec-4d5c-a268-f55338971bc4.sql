...
SELECT
  ...
  CASE 
    WHEN draft_data->>'heroImageUrl' IS NOT NULL AND draft_data->>'heroImageUrl' NOT LIKE 'data:%' 
    THEN draft_data->>'heroImageUrl'
    ELSE NULL
  END AS hero_image_url,
  CASE
    WHEN draft_data->'heroImageUrls' IS NOT NULL AND jsonb_array_length(COALESCE(draft_data->'heroImageUrls', '[]'::jsonb)) > 0
